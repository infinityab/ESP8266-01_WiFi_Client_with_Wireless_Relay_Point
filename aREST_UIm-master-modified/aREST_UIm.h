...
// Using ESP8266 ?
#if defined(ESP8266)
#include "stdlib_noniso.h" // stdlib_noniso.h
#endif
...
void pulse(int pin){
  ...
  // Set in button array
  pulses[pulses_index] = pin;
  pulses_index++;

}

// Create Wireless button
void wireless(int pin){

  // Set pin as output
 // pinMode(pin,OUTPUT);	// in wireless mode pin wireless socket #

  // Set in button array
  wirelesss[wireless_index] = pin;
  wireless_index++;
...
virtual void root_answer() {
    ...
    for (int i = 0; i < buttons_index; i++) {
      ...
      addToBuffer("<div class=\"col-md-2\"><button class=\"btn btn-block btn-lg btn-primary\" id='btn_off");
      addToBuffer(buttons[i]);
addToBuffer("'>Relay"+String(i+1)+" On</button></div>");
      addToBuffer("<div class=\"col-md-2\"><button class=\"btn btn-block btn-lg btn-danger\" id='btn_on"); // btn-danger\" id='btn_off");

    addToBuffer(buttons[i]);
addToBuffer("'>Relay"+String(i+1)+" Off</button></div>");
...
addToBuffer("</div>");
    ...
    for (int i = 0; i < pulses_index; i++) {
      ...
      addToBuffer(pulses[i]);
	 addToBuffer("'>Pulse"+String(i+1)+" On</button></div>");
	 addToBuffer("</div>");
    }


    // Wireless Buttons UI
    for (int i = 0; i < wireless_index; i++) {
      addToBuffer("<div class=\"row\">");
      addToBuffer("<div class=\"col-md-2\"><button class=\"btn btn-block btn-lg btn-primary\" id='btn_off");
     addToBuffer(wirelesss[i]);
	addToBuffer("'>Wireless"+String(i+1)+" On</button></div>");
     addToBuffer("<div class=\"col-md-2\"><button class=\"btn btn-block btn-lg btn-danger\" id='btn_on"); 
    	addToBuffer(wirelesss[i]);
	addToBuffer("'>Wireless"+String(i+1)+" Off</button></div>");
     addToBuffer("</div>");
    }
    ...
    // Pulse Buttons JavaScript
    for (int i = 0; i < pulses_index; i++) {
      addToBuffer("$('#btn_on");
      addToBuffer(pulses[i]);
      addToBuffer("').click(function() {$.getq('queue','/pulse/");
      addToBuffer(pulses[i]);
      addToBuffer("/1');});");
  }


    // Wireless Buttons JavaScript
    for (int i = 0; i < wireless_index; i++) {
      addToBuffer("$('#btn_on");
      addToBuffer(wirelesss[i]);
      addToBuffer("').click(function() {$.getq('queue','/wireless/");
      addToBuffer(wirelesss[i]);
      addToBuffer("/0');});");
      addToBuffer("$('#btn_off");
      addToBuffer(wirelesss[i]);
