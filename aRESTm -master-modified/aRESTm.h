...
void process(char c){
  ...
  if ((c == '/' || c == '\r') && state == 'u') {
     ...
     // If a pulse command has been received, process the data accordingly
     if (command == 'p' && pin_selected && state == 'u') {
     ...
     // If a wireless command has been received, process the data accordingly
     if (command == 'w' && pin_selected && state == 'u') {
	...
	// pulse command received ?
	if (answer.startsWith("pulse")) {command = 'p';}

	// Wireless command received ?
	if (answer.startsWith("wireless")) {command = 'w';}
...
// Digital selected
   ...
   if (command == 'w') {
	...
	if (state == 'w') {		// ********* Wireless **************
	   switch(((pin -21)*2) + value) {	// index to the correct wireless socket
		case 0:  				// pin is Socket #
    		mySwitch.send(w1_off, 32);  // OFF
    		break; 
  		case 1:  
    		mySwitch.send(w1_on, 32);    // ON
    		break;
  		case 2:  
    		mySwitch.send(w2_off, 32);  // OFF
    		break; 
  		case 3:  
    		mySwitch.send(w2_on, 32);    // ON
    		break;
  		case 4:  
  		mySwitch.send(w3_off, 32);  // OFF
  		break; 
		case 5:  
	    	mySwitch.send(w3_on, 32);    // ON
   		break;
		case 6:  				// pin is Socket #
    		mySwitch.send(w4_off, 32);  // OFF
    		break; 
  		case 7:  
    		mySwitch.send(w4_on, 32);    // ON
    		break;
  		case 8:  
    		mySwitch.send(w5_off, 32);  // OFF
    		break; 
  		case 9:  
    		mySwitch.send(w5_on, 32);    // ON
    		break;
  		case 10:  
  		mySwitch.send(w6_off, 32);  // OFF
  		break; 
		case 11:  
	    	mySwitch.send(w6_on, 32);    // ON
   		break;      
     		 default :
	    		mySwitch.send(0, 32);    // ON
  	  		break;
		}
 	   }
